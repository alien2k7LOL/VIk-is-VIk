...
const App = (() => {
  ...
  async function handleLogin(e) {
    ...
    if (!auth) {
      showAuthError('auth-error', 'Authentication is not set up. Add your Firebase config in index.html to enable sign in.');
      return;
    }
  ...
  async function handleSignup(e) {
    ...
    if (!auth) {
      showAuthError('auth-signup-error', 'Authentication is not set up. Add your Firebase config in index.html to enable sign up.');
      return;
    }
  ...
  async function handleGoogleSignIn() {
    const auth = getFirebaseAuth();
    if (!auth) {
      showAuthError('auth-error', 'Authentication is not set up. Add your Firebase config in index.html to enable Google sign in.');
      switchAuthTab('login');
      return;
    }
    clearAuthErrors();
  ...
  function renderTestPrepSection(tab) {
    ...
    state._tpTopic = null;
    state._tpStates = {};
    container.innerHTML = '<div class="tp-tiles">' + data.map(function(card, ci) {
  ...
  async function callOpenAI(prompt) {
    try {
      const messages = [
        { role: 'system', content: 'You are Prime Prep AI Tutor, an expert math teacher. Explain concepts clearly with step-by-step solutions. Use LaTeX notation with \\( \\) for inline math and \\[ \\] for display math. Be encouraging and thorough.' },
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) return 'AI service authentication failed. Please contact the site admin.';
      ...
      if (e.name === 'AbortError') return 'The request timed out. Please check your internet connection and try again.';
